...
def get_metric(pred_list, topk=10):
    ...
    for rank in pred_list:
        ...
        if rank < topk:
            ...
            HIT += 1.0
            # 判断是否有任何结果命中（即排名不为无穷大）
        if rank < float('inf'):
            HR += 1
    return HIT / len(pred_list), NDCG / len(pred_list), MRR / len(pred_list), HR / total_queries
...
def hit_rate_k(true_items, recommended_items, top_k):
    true_set = set(true_items)
    recommended_set = set(recommended_items[:top_k])
    hits = len(true_set & recommended_set)
